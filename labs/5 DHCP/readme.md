...
### Выполнение работы.
##### 1. Базовая настройка устройств
В виртуальной среде eve-ng добавляем объекты на рабочую область (R1, R2, SW1, SW2, VPC-A и VPC-B) и коммутируем их между собой в соответствии с графической схемой. В качестве образа роутеров R1 и R2 используем образ L3-adventerprisek9-m-15.4-2T.bin, в качестве образа коммутаторов SW1 и SW2 - образ L2-ADVENTERPRISEK9-M-15.2-20150703.bin, а для компьютеров задействуем стандартный шаблон Virtual PC (VPCS). 
Проводим минимальную базовую настроку окружения (имя хоста, отключаем резолвинг, включаем шифрование паролей, баннер на вход, запрет вывода консольных сообщений в процессе конфигурирования) и авторизации (пароли на привелигированный режим, на консоль и терминал )  на всех устройств на базе Cisco IOS:
```
enable
conf t
hostname SW1
...
##### 3. Производим настройку интерфейсов и VLAN  согласно схемы.  
На R1
```
R1(config)# int e0/1
...
Ethernet0/0 is up, line protocol is up
  ...
  MTU is 1500 bytes
  Helper address is 10.0.0.1    <-----Видим что пересылка на 10.0.0.1 включена
 ```
...
### Задание:
1.  Провести первоначальную базовую настройку.  
2.  Проверяем работу SLAAC
3. Настраиваем и проверяем DHCPv6 Stateles сервер на R3
4. Настраиваем и проверяем DHCPv6 Stateful сервер на R3
5. Настраиваем и проверяем работу пересылки DHCPv6
...
Пример нидже для R3,  на R4 делаем аналогично:
``
R3(config)#int e0/1
...
##### 3. Настраиваем и проверяем DHCPv6 stateless сервер на R3
...
```
R13(config)#int e0/0
R13(config-if)#ipv6 enable 
R13(config-if)#ipv6 address dhcp 
R13(config-if)#no shutdown
R13(config-if)#do show ipv6 interface brief
Ethernet0/0            [up/up]
...
Ethernet0/3            [administratively down/down]
    unassigned
R13(config-if)#do show ipv6 dhcp interface e0/0 
...
##### 4.  Настройка R3 для работы DHCPv6 в режиме stateful
...
R3(config-if)#exit
```
##### 5. Включение пересылки DHCPv6 и проверка работы DHCPv6 Stateful
...
Ethernet0/0 is in client mode
  ...
  Address Rapid-Commit: disabled
```
Смотрим статус на сервере R3:
```
R3#show ipv6 dhcp pool 
DHCPv6 pool: R3-Stateless
  DNS server: 2001:DB8:ACAD::254
  Domain name: otus-stateless.local
  Active clients: 0
DHCPv6 pool: R3-Stateful
  Address allocation prefix: 2001:DB8:ACAD:3:AAA::/80 valid 172800 preferred 86400 (1 in use, 0 conflicts)
  DNS server: 2001:DB8:ACAD::254
  Domain name: otus-stateful.local
  Active clients: 1
R3#show ipv6 dhcp binding 
Client: FE80::A8BB:CCFF:FE00:D020 
  DUID: 00030001AABBCC00D000
  Username : unassigned
  VRF : default
  IA NA: IA ID 0x00050001, T1 43200, T2 69120
    Address: 2001:DB8:ACAD:3:AAA:68B6:5A53:62DC
            preferred lifetime 86400, valid lifetime 172800
            expires at Mar 23 2022 10:27 PM (172735 seconds)
```

Пересылка DHCP работает, адрес DHCPv6 Stateful клиент получает. Работа выполнена.
